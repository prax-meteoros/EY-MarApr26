...
### Exercise 1: Set up a GitHub Codespace and verify GitHub Copilot

You'll use your personal GitHub account to create a Codespace. A Codespace is a cloud-hosted development environment that runs Visual Studio Code in the browser, with GitHub Copilot and the C# tooling pre-installed. Using a Codespace means you don't need to install Visual Studio Code or any extensions locally, and your GitHub identity is automatically available to the environment.

> [!NOTE]
> To complete this lab you need a personal GitHub account with an active GitHub Copilot subscription (Individual, Business, or the free plan available to verified students/teachers/maintainers). Your account also needs access to GitHub Codespaces (included free up to a monthly quota on personal accounts).

Use the following steps to set up your Codespace:

1. Open a browser window.

1. Navigate to GitHub and sign in to your **personal** GitHub account:

    ```plaintext
    https://github.com/login
    ```

1. Verify that GitHub Copilot is enabled on your account.

    Navigate to the following URL and confirm that your Copilot subscription is active:

    ```plaintext
    https://github.com/settings/copilot
    ```

    If Copilot isn't enabled, start a free trial or subscribe before continuing.

1. Create a new blank Codespace.
    ...
    - Scroll to the bottom of the page and select the **blank template** link (or, if shown, choose **Create blank codespace**).
    - Leave the default **Branch**, **Region**, and **Machine type** values.
    - Select **Create codespace**.

    It takes 1-2 minutes for the Codespace to build and open. When it's ready, Visual Studio Code loads in your browser, already connected to the cloud container.

1. Wait for the Codespace to finish loading.

    The Codespace is ready when the Visual Studio Code interface is fully displayed and the integrated terminal is available at the bottom of the window.

1. Verify that the GitHub Copilot extensions are installed and signed in.

    On the left side menu, select **Extensions**, and then confirm that **GitHub Copilot** and **GitHub Copilot Chat** are both listed under the **Installed** section. Codespaces pre-installs these extensions and signs in automatically using the personal GitHub account that launched the Codespace.

    If you see a **Sign in to use GitHub Copilot** prompt, select **Sign in to GitHub** and select **Allow** on any authorization dialogs. Because your browser is already signed in to GitHub, the flow completes without asking for credentials.

1. Notice that the **Open Chat** button is visible at the top of the Visual Studio Code window (just to the right of the command center), and the **Copilot** status icon is visible in the bottom-right Status Bar.

    If the Chat view isn't available, confirm that the **GitHub Copilot Chat** extension is installed and enabled from the Extensions view.

#### Summary - Exercise 1

In this exercise, you created a Codespace using your personal GitHub account. Because Codespaces ships with Visual Studio Code, the GitHub Copilot extensions, and automatic GitHub authentication pre-configured, there was no local install step required. You're now ready to use GitHub Copilot to analyze and document the library application codebase.
...
Because you're working inside a Codespace, you can't use a browser download dialog or Windows File Explorer. Instead, you'll download the zip file directly into the Codespace using `curl` and extract it with `unzip` from the integrated terminal.

Use the following steps to prepare the library application:

1. Ensure that your Codespace is open and that you have access to the integrated terminal.

    If the terminal isn't visible, open it from the **Terminal** menu by selecting **New Terminal**. The terminal runs inside the Codespace's Linux container, so commands below are shell commands, not Windows commands.

1. In the terminal, change into the `/workspaces` directory and download the zip file using `curl`.

    Run the following commands, one at a time:

    ```bash
    cd /workspaces
    curl -L -o GuidedProjectApp.zip https://raw.githubusercontent.com/MicrosoftLearning/APL-2007-Accelerate-app-development-by-using-GitHub-Copilot/master/LearnModuleExercises/Downloads/GuidedProjectApp.zip
    ```

    The `-L` flag tells curl to follow redirects, and `-o` saves the response to a file with the given name. The download should complete in a few seconds.

1. Verify that the zip file was downloaded successfully.

    ```bash
    ls -lh GuidedProjectApp.zip
    ```

    You should see `GuidedProjectApp.zip` listed with a non-zero file size.

1. Extract the downloaded project files.

    ```bash
    unzip GuidedProjectApp.zip
    ```

    > [!NOTE]
    > If `unzip` is not installed in the Codespace image you're using, install it first with:
    >
    > ```bash
    > sudo apt-get update && sudo apt-get install -y unzip
    > ```

    After extraction, run `ls` to confirm that an **AccelerateDevGitHubCopilot** folder now exists under `/workspaces`. If the zip extracted into a different top-level folder name, rename it to `AccelerateDevGitHubCopilot` with `mv <folder> AccelerateDevGitHubCopilot` so the rest of the lab steps line up.

1. (Optional) Remove the zip file to keep the workspace tidy.

    ```bash
    rm GuidedProjectApp.zip
    ```

1. Open the **AccelerateDevGitHubCopilot** folder as the Codespace workspace.

    1. In Visual Studio Code, on the **File** menu, select **Open Folder**.

    1. In the path field, type `/workspaces/AccelerateDevGitHubCopilot` and then select **OK**.

    1. If prompted, select **Yes, I trust the authors**.

    The Codespace reloads with the library application as the active workspace. If you're prompted to install recommended extensions (for example, the **C# Dev Kit**), accept the suggestion - this enables the Solution Explorer view and the **Debug > Start New Instance** commands used later in this lab.

    > [!NOTE]
    > It can take 10-20 seconds for Visual Studio Code to load the project files and any required .NET extensions.
...
1. Confirm that you're signed in to your personal GitHub account in the browser.

    Because the Codespace was created from your personal account, your GitHub identity is already propagated into the Codespace environment and used by both Visual Studio Code's Source Control view and the integrated terminal's `gh` CLI - you don't need to sign in separately for the publish step.

1. Open a new browser tab, navigate to `https://github.com`, open your GitHub account menu, and then select **Your repositories**.

    Keep this tab open - you'll use it after publishing to confirm the new repository and to add the README file.

1. Configure Git in the Codespace with **your own** personal information.

    Switch to the Visual Studio Code tab for the Codespace and open the integrated terminal (**Terminal** menu > **New Terminal**).

    Codespaces often pre-populates these values. First check whether they're already set:
    ...
    If either value is empty or incorrect, set it with the following commands, replacing the placeholders with the email address associated with your personal GitHub account and your own name:
